...
from exercises.ex06 import invert, favorite_color, count, alphabetizer, update_attendance
import pytest
...
def test_invert_error() -> None:
    ...
    with pytest.raises(KeyError):
        invert({"train": "rain", "train": "far", "heat": "seat", "glare": "pear"})
...
def test_favorite_color_one() -> None:
    """Testing invert dictionary of a dict with one pair."""
    assert favorite_color({"Mary": "green"}) == "green"

def test_favorite_color_capital() -> None:
    """Testing invert dictionary of a dict with four pairs and different cases."""
